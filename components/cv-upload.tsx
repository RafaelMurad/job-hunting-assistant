...
export function CVUpload({ onExtracted, onCancel }: CVUploadProps): JSX.Element {
  ...
  return (
    <Card className="border-2 border-dashed border-nordic-neutral-300 dark:border-gray-600">
      <CardContent className="p-6">
        ...
        <div
          ...
          className={`
            ...
            ${status === "idle" ? "cursor-pointer hover:bg-nordic-neutral-50 dark:hover:bg-gray-800" : ""}
            ${isDragOver ? "bg-fjord-50 dark:bg-fjord-900/30 border-fjord-400 dark:border-fjord-500" : ""}
            ${status === "error" ? "bg-clay-50 dark:bg-clay-900/30" : ""}
            ${status === "success" ? "bg-forest-50 dark:bg-forest-900/30" : ""}
          `}
        >
          {/* Idle State */}
          {status === "idle" && (
            <>
              <svg
                className="w-12 h-12 text-nordic-neutral-400 dark:text-gray-500 mb-4"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
              ...
              <p className="text-nordic-neutral-900 dark:text-gray-100 font-medium mb-1">
          ...
          {(status === "uploading" || status === "processing") && (
            <>
              ...
              <p className="text-sm text-nordic-neutral-500 dark:text-gray-400">{fileName}</p>
              <p className="text-xs text-nordic-neutral-400 dark:text-gray-500 mt-2">
          ...
          {status === "success" && (
            <>
              <svg
                className="w-12 h-12 text-forest-600 dark:text-forest-400 mb-4"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
              ...
              <p className="text-forest-900 dark:text-forest-100 font-medium mb-1">
          ...
          {status === "error" && (
            <>
              <svg
                className="w-12 h-12 text-clay-600 dark:text-clay-400 mb-4"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
              ...
              <p className="text-clay-900 dark:text-clay-100 font-medium mb-1">Upload failed</p>
              <p className="text-sm text-clay-700 dark:text-clay-300 mb-4">{error}</p>
